...
const LeaveSummaryCart = () => {
  ...
  const retriveAttendenceSummaryHandler = async ()=>{
    try {
      const response = await retriveAttendenceSummaryApi(`startDate=${getMonthStartAndEndDates().start}&endDate=${getMonthStartAndEndDates().currentDate}`)
...
export default LeaveSummaryCart